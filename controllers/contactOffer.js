...
export async function sendContactOffer(req, res, next) {
  const { firstname, email, company, toEmail, offerTitle, userId } = req.body;

  if (!firstname || !email || !toEmail || !offerTitle || !userId) {
    const error = new Error('All fields, including userId, are required');
    error.status = 400;
    return next(error);
  }
  ...
  try {
    ...
    // Check if contact attempts need to be reset (new day)
    const now = new Date();
    const lastReset = new Date(user.contactAttempts?.lastReset || now);
    const isNewDay =
      now.getDate() !== lastReset.getDate() ||
      now.getMonth() !== lastReset.getMonth() ||
      now.getFullYear() !== lastReset.getFullYear();

    if (isNewDay || !user.contactAttempts) {
      user.contactAttempts = { count: 0, lastReset: now };
    }

    // Check if user has exceeded daily contact limit
    if (user.contactAttempts.count >= MAX_CONTACT_ATTEMPTS) {
      const error = new Error('You have reached the daily limit of 3 contact attempts.');
      error.status = 429;
      return next(error);
    }
    ...
    res.status(200).json({
      message: 'Email sent and contact request saved successfully',
      attemptsLeft: MAX_CONTACT_ATTEMPTS - user.contactAttempts.count,
    });
  } catch (error) {
    console.error('Error in sendContactOffer:', error);
    error.status = 500;
    error.message = 'Failed to send email or save contact request';
    next(error);
  }
}
